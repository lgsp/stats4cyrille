...
# Configure Jupyter kernels (Python 3 by default for us)
jupyter_kernels = {
    "python": {
        "kernelspec": {
            "display_name": "Python",
...
jupyter_default_lang = 'python'

# Don't write metadata
jupyter_write_metadata = False

# Bash code blocks are to be treated in the same way as Python
# (we'll use Jupyter magic commands, e.g. %%bash or !ls)
jupyter_lang_synonyms = ["python", "bash"]

# Remove HTML only portions
...
project = u'Réseaux de neurones et Apprentissage profond (STA211)'
copyright = u'2021, Nicolas Thome - Cnam'
...
def setup(app):
    app.add_config_value('tpml2', '', True)
...
#latex_documents = [
#  ('index', 'rcp209.tex', u'RCP209',
#   u'Michel Crucianu, Marin Ferecatu, Nicolas Thome', 'manual'),
#]
